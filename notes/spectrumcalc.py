...
class matterspectrum(cl.powerclass):
    ...
    def load_gps(self, directory: str = 'semigps') -> list:
        ...
        if st.components and not st.neutrino:
            ...
            folder_gf = directory + '/pknl_components/gf'
            folder_qf = directory + '/pknl_components/qf'
            folder_pl = directory + '/pknl_components/pl'
    ...
    def pk_nl_pred(self, params: dict, **kwargs) -> np.ndarray:
        '''
        ...
        # use CLASS if we want
        if not self.emulator and st.components:

            rec = cl.powerclass.pk_nonlinear_components(self, params, **kwargs)

            return rec

        elif not self.emulator and not st.components:

            pk = cl.powerclass.pk_nonlinear(self, params, **kwargs)
    ...
    def int_pk_nl(self, params: dict, int_type: str = 'cubic', a_bary: float = 0, **kwargs) -> np.ndarray:
        '''
